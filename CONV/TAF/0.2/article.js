((w) => {
            ...
            <div class="${tag}__modal-body">        
                ...
                <div class="${tag}__rating-question">
                    <p class="${tag}__question-text">
                        How useful would you find it to be able to save articles of interest to you? <span class="${tag}__error-msg">* (This field is required)</span>
                    </p>
    ...
    const initVariation = () => {
        if (sessionStorage.getItem(`${tag}__article-survey`)) return;
        utils.waitUntil(() => document.querySelector('body'), 0).then((element) => {
            element.insertAdjacentHTML('beforeend', cvSurveyModal);
        });

        utils.waitUntil(() => document.querySelector('.dx-doi'), 0).then((element) => {
            element.insertAdjacentHTML('afterend', `<li class="${tag}__favorite">
                <a href="#">
    ...
    const cvRecordSurveyData = (score, response, device, pageURL) => {
        fetch("https://docs.google.com/forms/d/e/1FAIpQLSeMg6Rk_fFmtEe5j0MtCabU4oPA-JTIulKmvwXUabAatEIX1A/formResponse", {
            method: "POST",
            mode: "no-cors",
            headers: {
            ...
            .then(() => {
                ...
                sessionStorage.setItem(`${tag}__article-survey`, true);
                setTracking();
            })
            .catch(error => {
