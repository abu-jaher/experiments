  ...
  try {
    ...
    const comparisonTableStickyNav = () => {
      ...
      if (targetElement.getBoundingClientRect().top <= calcPaddingTop) {
        ...
        if (document.querySelector(".monthly-activated")) {

          changeUrl(

            "<span>Contact us</span>",

            "https://www.box.com/quote?tc=monthly|box-297-v1",

            "<span>Learn more</span>",
            "https://www.box.com/enterprise-plus?tc=monthly|box-297-v1"
          ...
          changeUrl(
            ...
            "https://account.box.com/signup/enterprise-plus/buynow?tc=annual|box-297-v1",

            "<span>Contact us</span>",
            
            "https://www.box.com/quote?tc=annual|box-297-v1"
    ...
    const initPlanComparison = () => {
      ...
      body.querySelectorAll('.pricing-features--plan-buttons .button-link[href*="https://www.box.com/enterprise-plus"],.pricing-features--plan-buttons .button-secondary[href*="https://www.box.com/enterprise-plus"],.pricing-features--plan-buttons .button-primary[href*="/enterprise-plus"],.pricing-features--plan-buttons .button-primary[href*="/enterprise-plus"]').forEach((element) => {
    ...
    const helper = {
      ...
      checkTcParam: (selector,trigger)=>{
        const interval = setInterval(() => {
          ...
          const elements = document.querySelectorAll(selector);
          const elementsArray = Array.from(elements);         
          if (elements.length > 0 && elementsArray.some(element => element.href.indexOf('box-297-v1') > -1)) {
            clearInterval(interval);
          }
        },100);
        setTimeout(() => {
      ...
      addTCparam: ()=> {
        ...
        for (let i = 0; i < cta.length; i++) {
          ...
          if (url.indexOf('annual') > -1 || url.indexOf('monthly') > -1) {
            if (url.indexOf('|box-297-v1') > -1) return;
...
try {
  ...
  const priceSectionForEnterprisePlus = `
        ...
        <b
          ><span class="monthly-price--wrapper"
            ><span
              class="monthly-price"
  ...
  // please work on buy now cta link and text content dont update any previous links from client side 2 new button and hide control button for safety
  /* Variation Init */
  const init = () => {
    document
      .querySelector(
