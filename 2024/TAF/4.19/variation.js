((w) => {
	...
	const log = qa ? Function.prototype.bind.call(console.log, console, '[CONV] TAF 4.19 |') : () => { };
	...
	const initVariation = () => {
		utils.waitUntil(() => document.querySelector('.first-top-navigation .topnav-browse-button'), 0).then((element) => {
		...
		utils.waitUntil(() => document.querySelector('.navigation--item__dropdown-content-mobile'), 0).then((element) => {
